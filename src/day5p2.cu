...
__device__ int find_ordering(const int *ordering, int orderingCount, int first, int second) {
    int ret = -1;
    for (int k = 0; k < orderingCount; ++k) {
        if (first == ordering[2 * k] && second == ordering[2 * k + 1]) {
            ret = k;
            break;
        }
    }
...
__device__ bool check_update(const int *ordering, int orderingCount, const int *update, int updateLen) {
    ...
    for (int i = 0; i < updateLen - 1; ++i) {
        for (int j = i + 1; j < updateLen; ++j) {
            if (find_ordering(ordering, orderingCount, update[j], update[i]) != -1) {
                ret = false;
            }
        }
    }
...
__device__ void fix_update(const int *ordering, int orderingCount, int *fixedUpdate, int updateLen) {
    for (int i = 1; i < updateLen - 1; ++i) {
        for (int j = 0; j < updateLen - i; ++j) {
            if (find_ordering(ordering, orderingCount, fixedUpdate[j + 1], fixedUpdate[j]) != -1) {
                int tmp = fixedUpdate[j];
                fixedUpdate[j] = fixedUpdate[j + 1];
                fixedUpdate[j + 1] = tmp;
            }
        }
    }
...
__global__ void check_updates(const int *ordering, int orderingCount, const int *updates, int updatesCount,
    ...
    int partialResult = 0;
    int *fixedUpdate = new int[maxUpdateLen];

    while (updateIdx < updatesCount) {
        if (!check_update(ordering, orderingCount, &updates[updateIdx * maxUpdateLen], updatesLen[updateIdx])) {
            memcpy(fixedUpdate, &updates[updateIdx * maxUpdateLen], updatesLen[updateIdx] * sizeof(int));
            fix_update(ordering, orderingCount, fixedUpdate, updatesLen[updateIdx]);
            partialResult += fixedUpdate[updatesLen[updateIdx] / 2];
        }
